...
public class RecordarPlatoActivity extends AppCompatActivity implements PlatoRecordarAdapter.onItemClickListener, PlatoRecordarAdapterBd.onItemClickListenerBd {
    ...
    private String email,nombre,imagen;
    ...
    public void onItemClick(int position) {
        if (position < 1) {
            ...
            intent = new Intent();
            intent.putExtra("nuevoPlato", true);
            ...
            intent = new Intent();
            intent.putExtra("recordarNuevo", listaPlatos.get(position));
            setResult(Activity.RESULT_OK, intent);
            finish();
        }
    }
    ...
    public void onItemClickBd(int position) {
        intent = new Intent();
        plato = new Plato(listaPlatosBd.get(position).getNombre(),listaPlatosBd.get(position).getDescripcion(),listaPlatosBd.get(position).getPrecio(),0,listaPlatosBd.get(position).getUrlImage(),false,new ArrayList<>());
        intent.putExtra("recordarNuevo", plato);
        setResult(Activity.RESULT_OK, intent);
        finish();
    }
}
